...
const DelayPrediction = () => {
  ...
  useEffect(() => {
    const state = location.state as { allocation: AllocationData } | null;
    if (state?.allocation) {
      setOriginalData(state.allocation);
      setActiveEmployees([...state.allocation.employees]);
      setRemovedEmployees([]);
      setBudgetPercent(100);
      setDeadlineWeeks(state.allocation.deadline_weeks);
    }
  }, [location.state]);
